...
let printMeA = debounce(printMe);

setTimeout(printMeA, 1000);
setTimeout(printMeA, 1000);
setTimeout(printMeA, 1000);
...
let printMeA = debounce(printMe, 5000);
...
// c) Extend debounce to allow the original debounced function printMe to take an argument msg which is included in the console.log statement.
    ...
    function debounce(callback, ms) {
        ...
        return function (message) {
            //  clearTimeout(timeout);
            timeout = setTimeout(() => callback(message), ms);
        };
    }

let printMeA = debounce(printMe, 1000);

setTimeout(printMeA, 1000,"Hello");
setTimeout(printMeA, 1000,"Hey");
setTimeout(() => printMeA('Hi'), 1000 );
